...
'''
#The above section of commented code was used when this project involved scrapping the web for the image datasets
...
#First grabbing test image from url
...
#Exporting the model
model.save(os.path.join('models', 'firstAidFireAlarmModel.h5'))
new_model = load_model(os.path.join('models', 'firstAidFireAlarmModel.h5'))
yhatnew = new_model.predict(np.expand_dims(resize/255, 0))
print('\nReloaded Model Prediction: ', yhatnew)
if yhatnew > 0.5:
    print('Reloaded model prediction is more than 0.5, so predicted class is first-aid kit')
else:
    print('Reloaded model prediction is less than 0.5, so predicted class is fire alarm')